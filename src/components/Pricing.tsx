          ...
          {pricingTiers.map((tier, i) => (
            <motion.article
              ...
              className={`relative flex flex-col border p-8 transition-colors duration-300 ${
                tier.featured
                  ? "border-accent/50 bg-accent/5 lg:-translate-y-2"
                  : "border-border hover:border-bone-muted/30"
              }`}
            >
              {tier.featured && (
                <span className="absolute -top-3 left-6 rounded-full bg-accent px-3 py-1 font-body text-[10px] font-semibold uppercase tracking-widest text-ground">
                  Most booked
                </span>
              )}
              <h3 className="font-display text-2xl text-bone">{tier.name}</h3>
              <p className="mt-2 font-display text-4xl text-accent-light">{tier.price}</p>
